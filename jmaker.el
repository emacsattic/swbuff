...
(defcustom jmaker-super-makefile-buffer-template
	'(
		"\"####\" 'n"
		"\"#### Java super Makefile automatically generated by jmaker \""
		"(jmaker-version-number) 'n"
		"\"#### Creation date: \" (current-time-string) 'n"
		"\"####\" 'n"
		"'n"
  ...
  "*Template for Java super Makefile.
Setting this variable defines a template instantiation
command `jmaker-insert-super-makefile', as a side-effect."
  :group 'jmaker
  :type '(repeat string)
  :set '(lambda (sym val)
					(defalias 'jmaker-insert-super-makefile
						(tempo-define-template "jmaker-super-makefile-buffer-template"
																	 (jde-gen-read-template val)
																	 nil
																	 "Insert a Java Makefile.super in the current buffer."))
					(set-default sym val)))
...
If the file `root'/Makefile.super already exists the command requires confirmation
to overwrite it."
	(interactive "DDirectory: ")
	(let ((makefile (concat (file-name-as-directory root) "Makefile.super")))
		(and (file-exists-p makefile)
				 (or (y-or-n-p (format "File `%s' exists; overwrite? " makefile))
						 (error "Canceled")))
		(with-current-buffer (find-file-noselect makefile)
...
(defun jmaker-jde-hook ()
	...
	(when (and jmaker-hook-jde-menu (not jde-xemacsp))
		...
		(define-key-after
			(lookup-key jde-mode-map [menu-bar JDE Generate])
			[jmaker-gen1] '("Super Makefile" . jmaker-generate-super-makefile)
			'jmaker-gen1)
		(define-key-after
